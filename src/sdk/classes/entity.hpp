...
public:
	...
	NETVAR("DT_BaseCombatCharacter", "m_hMyWeapons", weaponsRaw, unsigned long);
	NETVAR("DT_BasePlayer", "m_hViewModel[0]", viewModelHandle, void*);

	std::array<unsigned long, 48> weapons() {
		std::array<unsigned long, 48> weaponArray;
		unsigned long* weaponsPtr = weaponsRaw_ptr();
		for (int i = 0; i < 48; i++) {
			weaponArray[i] = weaponsPtr[i];
		}
		return weaponArray;
	}
