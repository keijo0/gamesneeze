...
bool Interfaces::init() {
	...
	if (instructionAddr) {
		...
		Log::log(WARN, " PlayerResource pattern not found");
	}

    // Updated renderBeams pattern from ecl-master
    uintptr_t renderBeamsPat = PatternScan::findFirstInModule("/client_client.so", "4C 89 F6 4C 8B 25 ? ? ? ? 48 8D 05");
    if (renderBeamsPat) {
        renderBeams = **relativeToAbsolute<ViewRenderBeams***>(renderBeamsPat + 6);
        Log::log(LOG, " renderBeams %lx", (uintptr_t)renderBeams);
    } else {
        Log::log(WARN, " renderBeams pattern not found, trying fallback");
        // Fallback pattern
        renderBeams = *relativeToAbsolute<ViewRenderBeams**>(PatternScan::findFirstInModule("/client_client.so", "48 8D 05 ? ? ? ? 48 8B 38 48 8B 07 FF 10 48 8D 05 ? ? ? ? 48 8B 38") + 3);
        Log::log(LOG, " renderBeams (fallback) %lx", (uintptr_t)renderBeams);
    }
