...
inline QAngle toAngle(const Vector& v) {
	QAngle angles;
	float yaw, pitch;
	...
	if (v.y == 0.f && v.x == 0.f) {
		...
		yaw = atan2f(v.y, v.x) * 57.295779513082f;
		if (yaw < 0.f)
			yaw += 360.f;

		pitch = atan2f(-v.z, v.Length2D()) * 57.295779513082f;
		if (pitch < 0.f)
			pitch += 360.f;
	}
...
inline float Convert(float degree) {
	return (degree * (M_PI / 180));
}

inline float Convert2(float radians) {
