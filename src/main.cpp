...
void MainThread() {
    MARKER("gamesneeze-injected");
    // Remove serverbrowser wait loop - may be causing crash
...
// Removed constructor to prevent double initialization with gamesneeze_init
...
/* Explicit init function for gdb injection */
extern "C" __attribute__((visibility("default"))) __attribute__((used)) __attribute__((externally_visible)) void gamesneeze_init() {
	std::thread mainThread(MainThread);
	mainThread.detach();
}

// Test if library is loaded at all
__attribute__((constructor)) 
void test_constructor() {
    FILE* fp = fopen("/tmp/gamesneeze-loaded", "w");
    if (fp) {
        fprintf(fp, "library-loaded\n");
        fclose(fp);
    }
}
