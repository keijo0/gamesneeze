...
void Menu::onSwapWindow(SDL_Window* window) {
    if (!initialised) {
        MARKER("menu-init-start");
        Log::log(LOG, "Initializing menu...");
        gl3wInit();
        IMGUI_CHECKVERSION();
        ImGui::CreateContext();
        ...
        initialised = true;
        MARKER("menu-init-done");
        Log::log(LOG, "Menu initialized!");
    }
    ...
    if (ImGui::IsKeyPressed(SDL_SCANCODE_DELETE, false)) {
        MARKER("delete-key-pressed");
        Log::log(LOG, "DELETE key pressed, toggling menu");
        Config::reloadCfgList();
        Menu::open = !Menu::open;
        Log::log(LOG, "Menu open state: %d", Menu::open);
        MARKER("menu-toggled");
    }
