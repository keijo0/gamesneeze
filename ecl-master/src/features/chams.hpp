...
namespace Chams {
    ...
    inline IMaterial* createMaterial(const char* materialName, const char* materialType, const char* material) {
        ...
        if (!initKeyValues) return nullptr;
        initKeyValues(keyValues, materialType);

        typedef void (*LoadFromBuffer)(KeyValues*, const char*, const char*, void*, const char*, void*);
        static LoadFromBuffer loadFromBuffer = (LoadFromBuffer)Memory::patternScan("/client_client.so", "55 48 89 E5 41 57 41 56 41 55 41 54 53 48 83 EC 18 48 85");
        if (!loadFromBuffer) return nullptr;
